...
export const submitCustomOrderInquiry = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    if (!supabaseUrl || !supabaseServiceKey) {
      throw new Error('Server is not configured to receive orders. Please call or text Joe.');
    }
    ...
    if (error || !inserted) {
      ...
      throw new Error('We could not save your order. Please text Joe at 702-460-9190.');
    }
